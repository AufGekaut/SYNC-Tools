...
system('title [SYNC] - Discord Spammer')
lock = Lock()
session = Session()
...
class Main:
    def __init__(self):
        ...
        print("""%s                                       ___  _ ____ ___  ____ _  _ _  _ ____ ____ 
                                       |  \ | [__  |__] |__| |\/| |\/| |___ |__/ 
                                       |__/ | ___] |    |  | |  | |  | |___ |  \ \n\n                                                %sFastest Discord Spammer https://discord.gg/n5Pzmjx2 .\n SYNC - DISPAMMER\n""" % (red(), reset()))
    ...
    def start(self):
        ...
        print(' %s[%s1%s] Join Server ( Risk to use "need phone phone Verifier" (soon) )\n [%s2%s] Spam Server\n\n%s> %sSelect an option%s: ' % (red(), reset(), red(), reset(), red(), reset(), red(), reset()), end='')
        ...
        if self.option.upper() in ['1', 'JOIN SERVER']:
            ...
            system('title [SYNC] - Discord Spammer ^| Exiting . . .')
            sleep(3)
            _exit(0)
    ...
    def join_title(self):
        while self.variables['completed'] < len(self.variables['tokens']):
            system('title [SYNC] - Discord Spammer ^| Token: %s/%s (%s%%) ^| Retries: %s' % (self.variables['completed'], len(self.variables['tokens']), round(((self.variables['completed'] / len(self.variables['tokens'])) * 100), 3), self.variables['retries']))
            sleep(0.4)
        system('title [SYNC] - Discord Spammer ^| Token: %s/%s (%s%%) ^| Retries: %s' % (self.variables['completed'], len(self.variables['tokens']), round(((self.variables['completed'] / len(self.variables['tokens'])) * 100), 3), self.variables['retries']))

    def spam_title(self):
        while True:
            system('title [SYNC] - Discord Spammer ^| Sent: %s ^| Retries: %s' % (self.variables['sent'], self.variables['retries']))
            sleep(0.4)
